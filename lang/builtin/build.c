...
#define LIST_COUNT (sizeof(LIST) / sizeof(BuiltinInfo))

// Dict Methods
const BuiltinInfo DICT[] = {
    {DICT_GET, "get", 1, 2},
    {DICT_KEYS, "keys", 0, 0},
...
#define DICT_COUNT (sizeof(DICT) / sizeof(BuiltinInfo))

// Set Methods
const BuiltinInfo SET[] = {
    {SET_ADD, "add", 1, 1},
...
#define SET_COUNT (sizeof(SET) / sizeof(BuiltinInfo))

// String Methods
const BuiltinInfo STRING[] = {
    {STR_UPPER, "upper", 0, 0},
    {STR_LOWER, "lower", 0, 0},
...
#define STR_COUNT (sizeof(STRING) / sizeof(BuiltinInfo))

// Int/Bool Methods
const BuiltinInfo INT[] = {
    {NUM_BIT_LENGTH, "bit_length", 0, 0},
    {NUM_BIT_COUNT, "bit_count", 0, 0},
};

#define INT_COUNT (sizeof(INT) / sizeof(BuiltinInfo))

// Float Methods
const BuiltinInfo FLOAT[] = {
    {NUM_IS_INTEGER, "is_integer", 0, 0},
    {NUM_HEX, "hex", 0, 0},
};

#define FLOAT_COUNT (sizeof(FLOAT) / sizeof(BuiltinInfo))

// Range Methods
const BuiltinInfo RANGE[] = {
    {RANGE_COUNT_METHOD, "count", 1, 1},
    {RANGE_INDEX, "index", 1, 1},
};

#define RANGE_COUNT (sizeof(RANGE) / sizeof(BuiltinInfo))
...
void initializeRegistry(Dict *dict, const BuiltinInfo *registry, size_t count)
{
   if (!dict || !registry)
      return;
...
void initializeBuiltins(Runtime *rt)
{
   ...
   initializeRegistry(methods->list, LIST, LIST_COUNT);

   // Initialize dict methods
   initializeRegistry(methods->dict, DICT, DICT_COUNT);

   // Initialize set methods
   initializeRegistry(methods->set, SET, SET_COUNT);

   // Initialize string methods
   initializeRegistry(methods->str, STRING, STR_COUNT);

   // Initialize int/bool methods
   initializeRegistry(methods->atom, INT, INT_COUNT);

   // Initialize float methods
   initializeRegistry(methods->real, FLOAT, FLOAT_COUNT);

   // Initialize range methods
   initializeRegistry(methods->range, RANGE, RANGE_COUNT);

   // Initialize core builtins (print, len, int, etc.)
   initializeRegistry(
       methods->builtin, BUILTIN, BUILTIN_COUNT //
   );
}
...
Data *dispatchListMethod(BuiltinType type, Data *object, List *args, Dict *kwargs, Runtime *rt)
{
   ...
   {
   case LIST_APPEND:
      if (list_append(cloneData(ARG(0)), list) == LIST_ERROR)
      {
   ...
   case LIST_INSERT:
   {
      ...
      void *value = ARG(1);
      if (list_insert(list, index, cloneData(value)) == LIST_ERROR)
      {
   ...
   case LIST_REMOVE:
   {
      if (list_remove(ARG(0), list, data_free) == LIST_ERROR)
      {
         throw_error(
             ERROR_VALUE, "%s not in list",
             dataTostring((Data *)ARG(0)));
      }
      return createData(TYPE_NONE, NULL);
   }
   ...
   case LIST_INDEX:
   {
      ...
      return createData(TYPE_INT, &idx);
   }
...
Data *dispatchDictMethod(BuiltinType type, Data *object, List *args, Dict *kwargs, Runtime *rt)
{
   ...
   case DICT_GET:
   {
      ...
      void *result = dict_get(ARG(0), dict);
      if (result)
         return (Data *)cloneData(result);
   ...
   case DICT_VALUES:
   {
      ...
      values->length = dict_values(dict, values->items);
      values->end = values->length - 1;
   ...
   case DICT_ITEMS:
   {
      ...
      for (int i = 0; i < dict->size; i++)
      {
         ...
         while (pair)
         {
            List *tuple = list_create(2);
            list_append(cloneData(pair->key), tuple);
            list_append(cloneData(pair->value), tuple);
            Data *data = createData(TYPE_LIST, tuple);
            data->ref->isEvaluated = 1;

            list_append(data, items);
            pair = pair->next;
         }
      }
      Data *result = createData(TYPE_LIST, items);
      result->ref->isEvaluated = 1;
      return result;
   }
...
Data *dispatchSetMethod(BuiltinType type, Data *object, List *args, Dict *kwargs, Runtime *rt)
{
   ...
   {
   case SET_ADD:
      set_add(cloneData(ARG(0)), set);
      return createData(TYPE_NONE, NULL);

   case SET_REMOVE:
   {
      int removed = set_remove(ARG(0), set);
      if (!removed)
      {
         throw_error(ERROR_KEY, dataTostring(ARG(0)));
         return createData(TYPE_NONE, NULL);
      }
      return createData(TYPE_NONE, NULL);
   }
   ...
   case SET_HAS:
      return createData(TYPE_BOOL, &(int){set_has(ARG(0), set)});
...
Data *dispatchNumMethod(BuiltinType type, Data *object, List *args, Dict *kwargs, Runtime *rt)
{
   ...
   case NUM_BIT_COUNT:
   {
      ...
      return createData(TYPE_INT, &count);
   }
...
Data *dispatchRangeMethod(BuiltinType type, Data *object, List *args, Dict *kwargs, Runtime *rt)
{
   ...
   case RANGE_COUNT_METHOD:
   {
      ...
      return createData(TYPE_INT, &(int){count});
   }
      ...
      if (range->step > 0)
      {
         if (search_val >= range->start && search_val < range->stop)
         {
            ...
            if (offset % range->step == 0)
            {
               ...
               return createData(TYPE_INT, &(int){index});
            }
         }
      }
