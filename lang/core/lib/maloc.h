 ...
 * data structures, and runtime Env objects for the custom programming language.
 *
 * This module handles:
 * - Data object creation with type-specific initialization
 * - Abstract Syntax Tree (AST) node creation and composition
 * - Runtime Env and method dictionary setup
...
/**
 * env_create - Create a new execution Env/scope
 * @parent: Parent Env (for scope chaining), or NULL for global scope
 *
 * Allocates a new Env with local variable dictionary and reference tracking.
 ...
 *
 * Returns: Pointer to new Env, or NULL on allocation failure
 */
 ...
 *
 * Sets up the complete runtime Env including:
 * - Global execution Env
 ...
 */
ParamInfo *createParam(char *name, ASTnode *defaultValue, int isVariadic);
 ...
 * The Env (env) and locals will be set later during function definition.
 *
 * Returns: Pointer to new Function, or NULL on allocation failure
 */
Function *createFunction(char *name, struct List *params, struct List *body, int async, int *depth);
 ...
 */
ForLoop *createFor(char *iterator, ASTnode *iterable, struct List *body, int *depth);
 ...
 */
WhileLoop *createWhile(ASTnode *condition, struct List *body, int *depth);
...
Import *createImport(char *module, struct List *items, char *alias, int all);
/**
...
Ternary *createTernary(ASTnode *cond, ASTnode *trueBlock, ASTnode *falseBlock);

Data *createLambda(struct List *params, ASTnode *expr, int lineno);

ModuleRegistry *registry_create(void);

EnumItem *createEnumItem(char *name, struct ASTnode *value);
