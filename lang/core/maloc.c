...
ParamInfo *createParam(char *name, ASTnode *defaultValue, int isVariadic)
{
   ...
   pinfo->hasDefault = defaultValue ? 1 : 0;
   pinfo->defaultValue = defaultValue;
   pinfo->isVariadic = isVariadic;
   return pinfo;
}

Function *createFunction(char *name, List *params, List *body, int async, int *depth)
{
   Function *func = malloc(sizeof(Function));
   if (!func)
      return NULL;

   func->name = name ? createData(TYPE_STR, name) : NULL;
   func->params = params;
   func->body = body;
   func->env = NULL;
   func->locals = NULL;
   func->isAsync = async; // ← add
...
ForLoop *createFor(char *iterator, ASTnode *iterable, List *body, int *depth)
{
...
{
   Ternary *ternary = malloc(sizeof(Ternary));
   if (!ternary)
      throw_error(ERROR_MEMORY, "Could not allocate memory");

   ternary->condition = cond;
   ternary->trueExpr = trueBlock;
   ternary->falseExpr = falseBlock;

   return ternary;
}

WhileLoop *createWhile(ASTnode *condition, List *body, int *depth)
{
...
Catch *createCatch(char *errorName, char *alias, List *statements)
{
   ...
   catch->alias = alias ? createData(TYPE_STR, alias) : NULL;
...
Data *createLambda(List *params, ASTnode *expr, int lineno)
{
   List *body = list_create(1);
   Statement *return_stmt = createStatement(STMT_RETURN, expr, lineno);
   if (!return_stmt)
   {
      list_free(body, NULL);
      return NULL;
   ...
   list_append(return_stmt, body);
   int depth = 0;
   Function *func = createFunction(NULL, params, body, 0, &depth);
   return createData(TYPE_LAMBDA, func);
}
...
ModuleRegistry *registry_create(void)
{
   ...
   reg->capacity = 16;
   reg->count = 0;
   reg->entries = malloc(sizeof(ModuleEntry) * reg->capacity);
   return reg;
}

EnumItem *createEnumItem(char *name, ASTnode *value)
{
   EnumItem *item = malloc(sizeof(EnumItem));
   item->name = strdup(name);
   item->hasValue = value != NULL;
   item->value = value;
   return item;
}

Enum *createEnum(char *name, List *items)
{
   Enum *e = malloc(sizeof(Enum));
   e->name = createData(TYPE_STR, name);
   e->items = items;
   return e;
}
