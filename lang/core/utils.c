...
unsigned long hash(void *key)
{
   ...
   default:
      fprintf(stderr, "DEBUG hash: type=%d ptr=%p\n", data->type, (void *)data);
      throw_error(ERROR_TYPE, "'%s' type is not unhashable",
...
static int is_str(Data *a)
{
   return a->type == TYPE_STR || a->type == TYPE_LOOKUP;
}
...
Data *getData(Data *key, Runtime *rt)
{
   ...
   if (rt->methods->builtin)
   {
      if (dict_has(key, rt->methods->builtin))
      {
         void *res = dict_get(key, rt->methods->builtin);
         return cloneData((Data *)res);
      }
   }
...
Data *getAttribute(Data *object, Data *name, Runtime *rt)
{
   ...
   case TYPE_STMT:
      throw_error(ERROR_ATTRIBUTE,
                  "database handle has no attribute '%s'", name->str);
   ...
   case TYPE_COND:
      throw_error(ERROR_ATTRIBUTE,
                  "thread handle has no attribute '%s'", name->str);
      return createData(TYPE_NONE, NULL);

   case TYPE_FUTURE:
      throw_error(ERROR_ATTRIBUTE,
                  "future has no attribute '%s'", name->str);
      return createData(TYPE_NONE, NULL);
   default:
      break;
   }
...
void setAttribute(Data *object, Data *attrName, Data *value)
{
   ...
   case TYPE_INSTANCE:
      dir = INST_PTR(object)->attributes;
      break;
   ...
   dict_insert(cloneData(attrName), value, dir);
}
