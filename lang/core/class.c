...
Data *handle_module_call_public(const ModuleFuncInfo *info,
                                List *args, Dict *kwargs, Runtime *rt)
{
   return handle_module_call((Data *)&(Data){
                                 .type = TYPE_MODULE, .any = (void *)info},
                             args, kwargs, rt);
}

Data *handle_class_call_public(Data *callee, List *args,
                               Dict *kwargs, Runtime *rt,
                               Runtime *src_rt)
{
   return handle_class_call(callee, args, kwargs, rt, src_rt);
}

Data *handle_module_call_vm(const void *info_ptr, List *args,
                            Dict *kwargs, Runtime *rt)
{
   const ModuleFuncInfo *info = (const ModuleFuncInfo *)info_ptr;
   Data tmp;
   tmp.type = TYPE_MODULE;
   tmp.any = (void *)info;
   return handle_module_call(&tmp, args, kwargs, rt);
}

Data *handle_class_call_vm(Data *callee, List *args,
                           Dict *kwargs, Runtime *rt)
{
   return handle_class_call(callee, args, kwargs, rt, NULL);
}

Data *call_function_kw(Data *func_data, List *args,
                       Dict *kwargs, Runtime *rt)
{
   if (!func_data || func_data->type != TYPE_FUNCTION)
      return createData(TYPE_NONE, NULL);
   return exec_func(FUNC_PTR(func_data), args, kwargs, NULL, rt);
}