...
{
   int matches = (!catch->errorName) || (catch->type == ERROR_ALL) ||
                 (catch->type == g_thrown.type);
   ...
   if (catch->alias)
   {
      // create data already created duplicates for string no need to strdup here
      Data *errMsg = createData(TYPE_STR, g_thrown.message);
      saveInEnvironment(catch->alias, errMsg, rt->env);
   }

   clear_error();
   Status status = executeBody(catch->statements, returns, rt);

   if (catch->alias)
      dict_pop(catch->alias, rt->env->vars);
...
Status handleException(Exception *except, void **returns, Runtime *rt)
{
   ...
   g_thrown.active = 1; // signal we're inside a try block
...
void initializeErrorClass(Runtime *rt)
{
   char *pName = "Error";
   Data *parentName = createData(TYPE_STR, pName);
   Class *pclass = createClass(parentName, NULL, NULL);
   Data *pclassData = createData(TYPE_CLASS, pclass);

   List *parents = list_create(1);
   list_append(pclassData, parents);

   for (int i = 0; i < (sizeof(table) / sizeof(ErrorEntry)); i++)
   {
      if (!table[i].name)
         continue;

      Data *name = createData(TYPE_STR, table[i].name);
      Class *class = createClass(name, NULL, parents);

      Data *classData = createData(TYPE_CLASS, (void *)class);
      saveInEnvironment(name, classData, rt->env);
   }
}